...
- **주간 보드** — 투두(상단 고정) + 타임라인(독립 스크롤) 분리 레이아웃. 현재 시간 표시, 좌우 화살표 키로 주 이동, 모바일 N열 보기(1/2/3 선택)
...
- **데일리 하이라이트** — 날짜 헤더 ★ 버튼으로 오늘의 핵심 목표 1개 설정 (Make Time 방법론, 모바일·데스크탑 통일)
...
- **모바일 최적화** — 소프트 키보드 레이아웃 밀림 방지(고정 바텀 시트 입력), 날짜 탭 7개 균등 분배(가로 스크롤 없음), TODO 텍스트 글자 단위 줄바꿈
